...
class Genetic_Algorithm(object):
    ...
    def select_parents(self, population,  num_parents):
        ...
        parents_by_fitness = population[0:int(num_parents * GCV.PARENTS_BY_FITNESS_PERCENT)]
        # select parents according to fitness
        proper_parents = [x for x in population if x.strength_raito >GCV.SAFETY_FACTOR][\
                0:int(num_parents * GCV.PROPER_PARENTS_PERCENT)]
        
        # select parents according to distance to 
        population_copy = copy.deepcopy(population)
        population_copy.sort(key = lambda c: np.abs(c.strength_raito - GCV.SAFETY_FACTOR))
    ...
    def crossover(self, parents, offspring_number):
        ...
        #for i in range(offspring_number):
            child_length = -1
            while(child_length < GCV.ANGLE_TYPE * 2):
                ...
                if(int(child_length/2) > GCV.ANGLE_TYPE):
    ...
    def mutation(self, offspring, mutation_percent=0.5):
        for i in range(len(offspring)):
            evu_dir= GCV.SAFETY_FACTOR - offspring[i].strength_raito

...
def get_initial_population():
    ...
    while(len(initial_population)<GCV.POPULATION_NUMBER):
        ...
        result = get_angle_list(GCV.ANGLE_TYPE, int(length/2))
...
if __name__ == "__main__":
    ...
    current_run_time = 0
    while(current_run_time < GCV.GA_RUNTIMES ):
        current_run_time = current_run_time + 1 
        parents = ga.select_parents(population,int(GCV.POPULATION_NUMBER * GCV.ELITIST_PERCENT))
        offspring = ga.crossover(parents, int(GCV.POPULATION_NUMBER*(1 - GCV.ELITIST_PERCENT)))
        ga.mutation(offspring)

        population[0:int(GCV.POPULATION_NUMBER * GCV.ELITIST_PERCENT)] = parents
        population[int(GCV.POPULATION_NUMBER * GCV.ELITIST_PERCENT):] = offspring
        population.sort(key = lambda c: c.fitness)
        ...
        GCV.FAILURE_CRITERIA = "max_stress"

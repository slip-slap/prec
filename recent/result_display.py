...
x_coordinate = list(np.arange(1,202,1))
...
ax1.set_ylim([0,20])
#ax1.plot(x_coordinate, result.coeff_low_fitness_, color=color)
ax1.plot(x_coordinate, result.coeff_mid_fitness_, color=color)
#ax1.plot(x_coordinate, result.coeff_high_fitness_, color=color)
...
ax2.set_ylim([0,10])
ax2.plot(x_coordinate, result.coeff_low_strength_ratio_
...
plt.annotate("$\sigma_1=8$",xy=(6,5.5),fontsize=13)
plt.annotate("$\sigma_2=3$",xy=(12,4.2),fontsize=13)
plt.annotate("$\sigma_3=0.5$",xy=(15,3),fontsize=13)

#plt.legend(custom_lines,['SR of','SR of'],loc="lower right",
